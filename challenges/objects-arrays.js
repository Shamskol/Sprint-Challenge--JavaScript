...
const contactInfo = [];
graduates.map(function(currentItem, index, array) {
  return contactInfo.push(`${currentItem.first_name} ${currentItem.email}`);
});
...
zooAnimals.forEach(function(currentItem, index, array) {
  return displayNames.push(
    `${currentItem.animal_name}, ${currentItem.scientific_name}`
  );
});
...
const lowCaseAnimalNames = zooAnimals.map((currentItem, index, array) => {
  return currentItem.animal_name.toLowerCase();
});
...
*/
const lowPopulationAnimals = zooAnimals.filter((currentItem, index, array) => {
  return currentItem.population < 5;
});
